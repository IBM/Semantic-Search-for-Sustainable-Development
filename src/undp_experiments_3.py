...
#Experiment 3: Combine matches from prior RIAs in semantic search (Append previous sentences to the target description).
combined_prior_matches = {}
vec = 1
for par_vec in par_vecs:
    for i in range(len(all_policy_documents)):
        ...
        combined_prior_matches[exclude_ria[0][:-5]+str(vec)] = [score_dict]
    vec += 1
    
i = 0
for key, val in combined_prior_matches.items():
    ...
    combined_prior_matches[key].append(match_by_sent)
    avg_new = avgMatches(match_by_sent, test_target_matches, 301)
    combined_prior_matches[key].append(avg_new)

    i+=1
    if i % 5 == 0:
        i = 0

combined_prior_matches['liberia_google'] = combined_prior_matches.pop('liberia1')
combined_prior_matches['liberia_nbow'] = combined_prior_matches.pop('liberia2')
combined_prior_matches['liberia_tfidf'] = combined_prior_matches.pop('liberia3')
combined_prior_matches['bhutan_google'] = combined_prior_matches.pop('bhutan_template21')
combined_prior_matches['bhutan_nbow'] = combined_prior_matches.pop('bhutan_template22')
combined_prior_matches['bhutan_tfidf'] = combined_prior_matches.pop('bhutan_template23')
combined_prior_matches['namibia_google'] = combined_prior_matches.pop('namibia_template21')
combined_prior_matches['namibia_nbow'] = combined_prior_matches.pop('namibia_template22')
combined_prior_matches['namibia_tfidf'] = combined_prior_matches.pop('namibia_template23')
combined_prior_matches['cambodia_google'] = combined_prior_matches.pop('cambodia_template21')
...
for key in examples:
    plt.plot(list(range(1, 101)), (np.asarray(sorted(combined_prior_matches['liberia_tfidf'][1][key]))*100)[:100], label = key)
